...
decadeModule.import((lib, game, ui, get, ai, _status) => {
	 ...
	 * ***请不要直接在这个文件添加动态皮肤！***
	 * 
	 * ***还请另行复制此文件并粘贴到本目录，然后改名为dynamicSkin.js！***
	 * 
	 * ***然后就可以随意在粘贴后的dynamicSkin.js中修改动态皮肤参数了！***
	 * 
	 * 十周年UI动皮使用说明：
	...
	const dynamicSkinTemplate = {
		...
		simashi: {
			桀骜睥睨: {
				name: 'skin_simashi_JieAoPiNi',
				x: [0, 0.04],
				y: [0, 0.3],
				scale: 0.85,
				angle: 15,
				background: 'skin_simashi_JieAoPiNi_bg.png'
			}
		},
		sunluyu: {
			娇俏伶俐: {
		...
		get re_baosanniang() {
			return this.baosanniang;
		},
		get re_daqiao() {
			return this.daqiao;
		},
		get re_diaochan() {
			return this.diaochan;
		},
		get re_huangyueying() {
			return this.huangyueying;
		},
		get re_panshu() {
			return this.panshu;
		},
		...
		get re_sunluban() {
			return this.sunluban;
		},
		get re_sunluyu() {
			return this.sunluyu;
		},
		get re_sunshangxiang() {
			return this.sunshangxiang;
		},
		get re_wangyi() {
			return this.wangyi;
		},
		get ol_xiaoqiao() {
			return this.re_xiaoqiao;
		},
		get re_xinxianying() {
			return this.xinxianying;
		},
		get ol_zhangchangpu() {
			return this.zhangchangpu;
		},
		get re_zhenji() {
			return this.zhenji;
		}
	};
	const dynamicSkin = decadeUI.dynamicSkin;

	if (dynamicSkin) Object.keys(dynamicSkinTemplate).forEach(key => {
		if (!dynamicSkin[key]) Object.defineProperty(dynamicSkin, key, Object.getOwnPropertyDescriptor(dynamicSkinTemplate, key));
	});
	else decadeUI.dynamicSkin = dynamicSkinTemplate;
});
